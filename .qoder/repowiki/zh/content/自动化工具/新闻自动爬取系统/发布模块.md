...
## 更新摘要
**变更内容**
- 更新了重复检测系统的实现，新增基于原始URL的重复检测机制
- 增强了内容块结构标准化，为每个语言的内容块添加_unique_key字段
- 改进了错误处理和日志记录系统
- 更新了发布器的详细实现分析
...
**更新** 本版本引入了基于原始URL的重复检测机制，显著提升了发布准确性，同时增强了内容块结构的标准化处理。
...
**更新** 发布器现在包含基于原始URL的重复检测机制，优先检查原始URL以避免重复发布。
...
**图表来源**
- [scripts/news-auto/crawler.ts:163-205](file://scripts/news-auto/crawler.ts#L163-L205)
- [scripts/news-auto/news-sources.config.ts:126-130](file://scripts/news-auto/news-sources.config.ts#L126-L130)
...
### 发布器（publisher.ts）
职责与流程
- **重复检测**：基于原始URL和标题双重检查，优先使用原始URL确保准确性。
- 分类解析：根据分类名查询分类 ID，建立引用关系。
- 图片上传：从 URL 下载图片并上传至 Sanity 资产库，生成引用。
- **内容块标准化**：为每个语言的内容块添加_unique_key字段，确保结构完整性。
...
**更新** 发布器现在包含基于原始URL的重复检测机制，这是最重要的改进之一。新的重复检测流程如下：

```mermaid
flowchart TD
Start(["开始重复检测"]) --> CheckURL{"检查原始URL"}
CheckURL --> |存在| QueryURL["查询原始URL是否已存在"]
QueryURL --> ExistsURL{"URL已存在？"}
ExistsURL --> |是| LogURL["记录URL重复"] --> ReturnTrue["返回重复"]
ExistsURL --> |否| CheckTitle["检查中文标题"]
CheckURL --> |不存在| CheckTitle
CheckTitle --> QueryTitle["查询中文标题是否已存在"]
QueryTitle --> ExistsTitle{"标题已存在？"}
ExistsTitle --> |是| LogTitle["记录标题重复"] --> ReturnTrue
ExistsTitle --> |否| LogPass["记录通过检测"] --> ReturnFalse["返回非重复"]
```

**图表来源**
- [scripts/news-auto/publisher.ts:14-35](file://scripts/news-auto/publisher.ts#L14-L35)

**更新** 内容块结构标准化：发布器现在为每个语言的内容块添加_unique_key字段，确保内容结构的完整性和可追踪性。
...
Pub->>Doc : 构建文档含_unique_key字段
Doc-->>Pub : 返回结果
```

**图表来源**
- [scripts/news-auto/publisher.ts:75-287](file://scripts/news-auto/publisher.ts#L75-L287)
- [lib/sanity/client.ts:1-30](file://lib/sanity/client.ts#L1-L30)
- [sanity/schemas/article.ts:1-265](file://sanity/schemas/article.ts#L1-L265)
...
**更新** 重复检测机制的优化减少了不必要的数据库查询，提高了整体性能。
...
- **通义千问 API 失败**
  ...
  - 参考路径：[scripts/news-auto/ai-processor.ts:23-62](file://scripts/news-auto/ai-processor.ts#L23-L62)
- **Sanity 写入失败**
  - 现象：发布器创建失败或图片上传异常。
  - 排查：检查 SANITY_API_TOKEN 与项目/数据集配置；确认分类是否存在；查看网络与 CDN 设置。
  - 参考路径：[lib/sanity/client.ts:1-30](file://lib/sanity/client.ts#L1-L30)，[scripts/news-auto/publisher.ts:44-72](file://scripts/news-auto/publisher.ts#L44-L72)
- **新闻源抓取失败**
  - 现象：RSS 解析错误或网页抓取返回空。
  - 排查：检查新闻源 headers（UA 等）、selector 与 RSS 地址；确认站点可用性。
  - 参考路径：[scripts/news-auto/crawler.ts:30-73](file://scripts/news-auto/crawler.ts#L30-L73)，[scripts/news-auto/news-sources.config.ts:126-130](file://scripts/news-auto/news-sources.config.ts#L126-L130)
- **时间窗口与配额限制**
  - 现象：定时任务未执行或提前结束。
  - 排查：确认北京时间换算与 ±90 分钟窗口；检查当日自动发布计数与上限配置。
  - 参考路径：[scripts/news-auto/scheduler.ts:67-94](file://scripts/news-auto/scheduler.ts#L67-L94)
- **重复发布问题**
  - 现象：日志提示已存在但未阻止发布。
  - 排查：确认新的基于原始URL的重复检测逻辑是否正常工作；检查数据库查询条件。
  - 参考路径：[scripts/news-auto/publisher.ts:14-35](file://scripts/news-auto/publisher.ts#L14-L35)
- **内容块结构问题**
  - 现象：Sanity Studio 中内容块显示异常。
  - 排查：确认发布器中是否正确添加了_unique_key字段；检查内容块结构。
  - 参考路径：[scripts/news-auto/publisher.ts:131-216](file://scripts/news-auto/publisher.ts#L131-L216)

**更新** 新增了基于原始URL的重复检测问题排查指南。

**章节来源**
- [scripts/news-auto/ai-processor.ts:23-62](file://scripts/news-auto/ai-processor.ts#L23-L62)
- [lib/sanity/client.ts:1-30](file://lib/sanity/client.ts#L1-L30)
- [scripts/news-auto/publisher.ts:44-72](file://scripts/news-auto/publisher.ts#L44-L72)
- [scripts/news-auto/crawler.ts:30-73](file://scripts/news-auto/crawler.ts#L30-L73)
- [scripts/news-auto/news-sources.config.ts:126-130](file://scripts/news-auto/news-sources.config.ts#L126-L130)
- [scripts/news-auto/scheduler.ts:67-94](file://scripts/news-auto/scheduler.ts#L67-L94)
- [scripts/news-auto/publisher.ts:14-35](file://scripts/news-auto/publisher.ts#L14-L35)
- [scripts/news-auto/publisher.ts:131-216](file://scripts/news-auto/publisher.ts#L131-L216)

## 结论
本发布模块通过清晰的分层设计与严格的配置管理，实现了从新闻源抓取到 AI 处理再到 Sanity 写入的全链路自动化。其核心优势在于：
- **可维护性强**：新闻源与配置独立文件，便于运维调整。
- **质量可控**：关键词过滤、内容长度与密度阈值、翻译回退策略。
- **稳健可靠**：**改进的重复检测机制**（基于原始URL优先检查）、限流与错误日志、时间窗口与配额控制。
- **结构标准化**：**内容块结构标准化**（添加_unique_key字段），确保数据完整性。

**更新** 本版本的重大改进包括基于原始URL的重复检测系统和内容块结构标准化，显著提升了发布准确性与数据完整性。
