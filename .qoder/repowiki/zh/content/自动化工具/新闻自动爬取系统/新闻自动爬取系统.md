# 新闻自动爬取系统

<cite>
**本文引用的文件**
- [scripts/news-auto/index.ts](file://scripts/news-auto/index.ts)
- [scripts/news-auto/crawler.ts](file://scripts/news-auto/crawler.ts)
- [scripts/news-auto/ai-processor.ts](file://scripts/news-auto/ai-processor.ts)
- [scripts/news-auto/publisher.ts](file://scripts/news-auto/publisher.ts)
- [scripts/news-auto/scheduler.ts](file://scripts/news-auto/scheduler.ts)
- [scripts/news-auto/config.ts](file://scripts/news-auto/config.ts)
- [scripts/news-auto/news-sources.config.ts](file://scripts/news-auto/news-sources.config.ts)
- [lib/sanity/client.ts](file://lib/sanity/client.ts)
- [sanity/schemas/article.ts](file://sanity/schemas/article.ts)
- [app/api/cron/news/route.ts](file://app/api/cron/news/route.ts)
- [scripts/test-news-workflow.ts](file://scripts/test-news-workflow.ts)
- [scripts/test-ai-image.ts](file://scripts/test-ai-image.ts)
- [scripts/test-ai-image-quick.ts](file://scripts/test-ai-image-quick.ts)
- [package.json](file://package.json)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本系统是一个面向LED行业的新闻自动爬取与发布平台，具备以下能力：
- 多源新闻采集：支持RSS与网页两种抓取方式，内置关键词过滤与去重
- AI智能处理：基于通义千问模型进行内容改写、摘要生成、关键词抽取与多语言翻译，**新增AI图像生成能力**
- 发布调度：基于北京时间的时间窗口与每日配额的发布控制
- 自动发布：将处理后的文章发布至Sanity内容管理平台，支持多语言内容结构与**AI生成配图**
- 定时任务：通过Vercel Cron触发自动化流程，具备安全校验与错误处理

## 项目结构
新闻自动爬取系统的核心代码位于 scripts/news-auto 目录，配合Sanity内容模型与Next.js API路由实现端到端自动化。

```mermaid
graph TB
subgraph "自动化脚本"
IDX["index.ts<br/>主流程编排"]
CRW["crawler.ts<br/>新闻抓取"]
AI["ai-processor.ts<br/>AI处理<br/>含图像生成"]
PUB["publisher.ts<br/>发布到Sanity<br/>含重复检测"]
SCH["scheduler.ts<br/>发布调度"]
CFG["config.ts<br/>全局配置"]
SRC["news-sources.config.ts<br/>新闻源配置"]
end
subgraph "外部服务"
SAN["lib/sanity/client.ts<br/>Sanity客户端"]
API["app/api/cron/news/route.ts<br/>定时触发API"]
end
subgraph "内容模型"
ART["sanity/schemas/article.ts<br/>文章Schema<br/>含SEO与多语言"]
end
subgraph "测试脚本"
TEST1["test-news-workflow.ts<br/>完整工作流测试"]
TEST2["test-ai-image.ts<br/>AI图像生成测试"]
TEST3["test-ai-image-quick.ts<br/>快速AI图像测试"]
end
IDX --> SCH
IDX --> CRW
IDX --> AI
IDX --> PUB
CRW --> SRC
CRW --> CFG
AI --> CFG
PUB --> SAN
SAN --> ART
API --> IDX
TEST1 --> AI
TEST2 --> AI
TEST3 --> AI
```

**图表来源**
- [scripts/news-auto/index.ts:1-92](file://scripts/news-auto/index.ts#L1-L92)
- [scripts/news-auto/crawler.ts:1-197](file://scripts/news-auto/crawler.ts#L1-L197)
- [scripts/news-auto/ai-processor.ts:1-375](file://scripts/news-auto/ai-processor.ts#L1-L375)
- [scripts/news-auto/publisher.ts:1-287](file://scripts/news-auto/publisher.ts#L1-L287)
- [scripts/news-auto/scheduler.ts:1-104](file://scripts/news-auto/scheduler.ts#L1-L104)
- [scripts/news-auto/config.ts:1-79](file://scripts/news-auto/config.ts#L1-L79)
- [scripts/news-auto/news-sources.config.ts:1-178](file://scripts/news-auto/news-sources.config.ts#L1-L178)
- [lib/sanity/client.ts:1-30](file://lib/sanity/client.ts#L1-L30)
- [sanity/schemas/article.ts:1-265](file://sanity/schemas/article.ts#L1-L265)
- [app/api/cron/news/route.ts:1-58](file://app/api/cron/news/route.ts#L1-L58)
- [scripts/test-news-workflow.ts:1-68](file://scripts/test-news-workflow.ts#L1-L68)
- [scripts/test-ai-image.ts:1-142](file://scripts/test-ai-image.ts#L1-L142)
- [scripts/test-ai-image-quick.ts:1-94](file://scripts/test-ai-image-quick.ts#L1-L94)

**章节来源**
- [scripts/news-auto/index.ts:1-92](file://scripts/news-auto/index.ts#L1-L92)
- [package.json:1-46](file://package.json#L1-L46)

## 核心组件
- 主流程编排：负责按顺序执行发布检查、抓取、AI处理、构建sourceMap与发布，并输出汇总信息
- 新闻抓取器：从RSS与网页两类源抓取，支持自定义headers、图片提取、关键词过滤与去重
- AI处理器：调用通义千问API完成内容改写、标题与摘要生成、多语言翻译、关键词抽取与SEO信息生成，**新增AI图像生成能力**
- 发布器：将处理后的文章发布到Sanity，包含**基于URL和标题的双重重复检测**、分类查询、图片上传与文档构建
- 调度器：基于北京时间的时间窗口与每日配额控制发布时机
- 配置中心：集中管理发布策略、关键词过滤、AI参数与目标语言，**新增内容质量阈值配置**
- 新闻源配置：独立维护新闻源列表，支持启用/停用、优先级、类型与请求头

**章节来源**
- [scripts/news-auto/index.ts:9-78](file://scripts/news-auto/index.ts#L9-L78)
- [scripts/news-auto/crawler.ts:155-196](file://scripts/news-auto/crawler.ts#L155-L196)
- [scripts/news-auto/ai-processor.ts:156-354](file://scripts/news-auto/ai-processor.ts#L156-L354)
- [scripts/news-auto/publisher.ts:14-35](file://scripts/news-auto/publisher.ts#L14-L35)
- [scripts/news-auto/scheduler.ts:67-103](file://scripts/news-auto/scheduler.ts#L67-L103)
- [scripts/news-auto/config.ts:6-79](file://scripts/news-auto/config.ts#L6-L79)
- [scripts/news-auto/news-sources.config.ts:46-178](file://scripts/news-auto/news-sources.config.ts#L46-L178)

## 架构总览
系统采用"定时触发 → 流程编排 → 数据采集 → AI处理 → 发布入库"的流水线式架构。定时任务通过Vercel Cron调用Next.js API，API层进行权限校验后启动自动化流程；流程内部通过配置中心统一管理策略，通过独立的新闻源配置文件实现灵活扩展。

```mermaid
sequenceDiagram
participant Cron as "Vercel Cron"
participant API as "Next.js API"
participant IDX as "主流程(index.ts)"
participant SCH as "调度器(scheduler.ts)"
participant CRW as "抓取器(crawler.ts)"
participant AI as "AI处理器(ai-processor.ts)"
participant PUB as "发布器(publisher.ts)"
participant SAN as "Sanity"
Cron->>API : "GET /api/cron/news"
API->>API : "条件验证CRON_SECRET"
API->>IDX : "runNewsAutomation()"
IDX->>SCH : "shouldPublish()"
SCH-->>IDX : "{should,reason,remaining}"
IDX->>CRW : "crawlNews()"
CRW-->>IDX : "RawArticle[]"
IDX->>AI : "processArticles(articles)"
AI-->>IDX : "ProcessedArticle[]<br/>含AI图像"
IDX->>PUB : "publishArticles(processed, sourceMap)"
PUB->>SAN : "创建文章文档<br/>含封面图"
SAN-->>PUB : "返回结果"
PUB-->>IDX : "已发布数量"
IDX-->>API : "汇总结果"
API-->>Cron : "JSON响应"
```

**图表来源**
- [app/api/cron/news/route.ts:5-46](file://app/api/cron/news/route.ts#L5-L46)
- [scripts/news-auto/index.ts:9-78](file://scripts/news-auto/index.ts#L9-L78)
- [scripts/news-auto/scheduler.ts:67-94](file://scripts/news-auto/scheduler.ts#L67-L94)
- [scripts/news-auto/crawler.ts:155-196](file://scripts/news-auto/crawler.ts#L155-L196)
- [scripts/news-auto/ai-processor.ts:269-354](file://scripts/news-auto/ai-processor.ts#L269-L354)
- [scripts/news-auto/publisher.ts:75-286](file://scripts/news-auto/publisher.ts#L75-L286)

## 详细组件分析

### 主流程编排（index.ts）
- 功能要点
  - 输出运行时间（UTC与北京时间）
  - 调用调度器判断是否允许发布与剩余配额
  - 抓取新闻并按配额裁剪
  - 构建sourceMap用于发布时回填来源信息
  - 发布到Sanity并输出统计摘要
- 错误处理
  - 捕获异常并抛出，便于上层API返回错误

```mermaid
flowchart TD
START(["开始"]) --> CHECK["调用 shouldPublish()"]
CHECK --> DECIDE{"是否允许发布？"}
DECIDE --> |否| SKIP["跳过并退出"]
DECIDE --> |是| CRAWL["crawlNews() 抓取新闻"]
CRAWL --> EMPTY{"是否有新文章？"}
EMPTY --> |否| END1["结束"]
EMPTY --> |是| QUOTA["getPublishQuota() 限制数量"]
QUOTA --> PROCESS["processArticles() AI处理<br/>含图像生成"]
PROCESS --> MAP["构建 sourceMap"]
MAP --> PUBLISH["publishArticles() 发布到Sanity"]
PUBLISH --> SUMMARY["输出统计摘要"]
SUMMARY --> END2["结束"]
```

**图表来源**
- [scripts/news-auto/index.ts:9-78](file://scripts/news-auto/index.ts#L9-L78)

**章节来源**
- [scripts/news-auto/index.ts:9-78](file://scripts/news-auto/index.ts#L9-L78)

### 新闻抓取器（crawler.ts）
- 数据源类型
  - RSS：解析RSS条目，提取标题、链接、内容、摘要、发布时间、图片
  - 网页：基于CSS选择器批量提取文章块，拼接相对链接与图片URL
- 关键策略
  - 关键词过滤：支持必需词与排除词
  - 去重：基于链接去重
  - 排序：依据新闻源优先级排序
- 反爬虫策略
  - 支持自定义headers（如User-Agent），合并默认UA与配置项
  - RSS解析支持自定义headers
  - 超时控制与错误捕获

```mermaid
flowchart TD
S["读取启用的新闻源"] --> LOOP{"遍历每个源"}
LOOP --> TYPE{"源类型？"}
TYPE --> |rss/web/rss+web| FETCH["fetchFromRSS/fetchFromWeb"]
FETCH --> MERGE["收集文章"]
MERGE --> DEDUP["去重按链接"]
DEDUP --> FILTER["关键词过滤"]
FILTER --> SORT["按优先级排序"]
SORT --> RET["返回最终文章列表"]
```

**图表来源**
- [scripts/news-auto/crawler.ts:155-196](file://scripts/news-auto/crawler.ts#L155-L196)
- [scripts/news-auto/news-sources.config.ts:126-144](file://scripts/news-auto/news-sources.config.ts#L126-L144)

**章节来源**
- [scripts/news-auto/crawler.ts:21-196](file://scripts/news-auto/crawler.ts#L21-L196)
- [scripts/news-auto/news-sources.config.ts:46-178](file://scripts/news-auto/news-sources.config.ts#L46-L178)

### AI处理器（ai-processor.ts）
- 处理流程
  - 内容改写：生成专业中文行业资讯
  - 标题与摘要：生成吸引人的标题与列表摘要
  - 多语言翻译：对标题、摘要与正文进行多语言翻译
  - 关键词抽取：提取英文SEO关键词
  - SEO信息：生成多语言metaTitle与metaDescription
  - **新增AI图像生成**：基于文章内容生成专业配图
- API调用
  - 通义千问API（qwen-turbo），支持超时与错误处理
  - **通义万相API（wanx-v1）**，用于AI图像生成
  - 环境变量：DASHSCOPE_API_KEY
- 限流与健壮性
  - 处理间隔延时，避免API限流
  - 单个语言翻译失败时的降级策略（回退到英文或中文）
  - **AI图像生成失败时的降级策略**

```mermaid
sequenceDiagram
participant A as "processArticle"
participant Q as "callQwen"
participant W as "generateAIImage"
participant R as "rewriteContent"
participant T as "translateContent"
participant K as "extractKeywords"
participant E as "generateExcerpt"
A->>R : "改写中文内容"
R-->>A : "中文正文"
A->>Q : "生成中文标题"
Q-->>A : "中文标题"
A->>E : "生成中文摘要"
E-->>A : "中文摘要"
A->>T : "翻译各语言正文/标题/摘要"
T-->>A : "多语言内容"
A->>K : "抽取英文关键词"
K-->>A : "关键词列表"
A->>W : "生成AI图像"
W-->>A : "图像URL或null"
A-->>A : "组装ProcessedArticle<br/>含AI图像"
```

**图表来源**
- [scripts/news-auto/ai-processor.ts:269-354](file://scripts/news-auto/ai-processor.ts#L269-L354)

**章节来源**
- [scripts/news-auto/ai-processor.ts:18-58](file://scripts/news-auto/ai-processor.ts#L18-L58)
- [scripts/news-auto/ai-processor.ts:87-154](file://scripts/news-auto/ai-processor.ts#L87-L154)
- [scripts/news-auto/ai-processor.ts:156-234](file://scripts/news-auto/ai-processor.ts#L156-L234)
- [scripts/news-auto/ai-processor.ts:236-354](file://scripts/news-auto/ai-processor.ts#L236-L354)

### 发布器（publisher.ts）
- 发布流程
  - **改进的重复检测**：按中文标题查询是否存在，**新增基于原始URL的重复检测**
  - 分类查询：根据分类别名获取Sanity分类ID
  - 图片上传：下载并上传封面图到Sanity资产
  - 文档构建：按多语言结构构建文章内容块
  - 创建文档：写入Sanity
- 错误处理
  - 重复、分类缺失、图片下载/上传失败均有相应日志与降级

```mermaid
flowchart TD
P["publishArticle"] --> DUPE["检查重复<br/>URL + 标题"]
DUPE --> |存在| ABORT["跳过并返回"]
DUPE --> |不存在| CAT["查询分类ID"]
CAT --> |无| FAIL["记录错误并返回"]
CAT --> IMG["上传封面图可选"]
IMG --> DOC["构建文章文档<br/>含AI图像"]
DOC --> CREATE["client.create()"]
CREATE --> OK["发布成功"]
```

**图表来源**
- [scripts/news-auto/publisher.ts:75-286](file://scripts/news-auto/publisher.ts#L75-L286)

**章节来源**
- [scripts/news-auto/publisher.ts:14-35](file://scripts/news-auto/publisher.ts#L14-L35)
- [scripts/news-auto/publisher.ts:75-286](file://scripts/news-auto/publisher.ts#L75-L286)
- [lib/sanity/client.ts:1-30](file://lib/sanity/client.ts#L1-L30)
- [sanity/schemas/article.ts:1-265](file://sanity/schemas/article.ts#L1-L265)

### 调度器（scheduler.ts）
- 时间窗口控制
  - 将UTC时间转换为北京时间（UTC+8）
  - 考虑Vercel Hobby套餐±1小时浮动，设置90分钟时间窗口
  - 支持本地测试模式绕过时间检查（通过环境变量）
- 配额管理
  - 查询当日已发布数量（仅统计自动来源）
  - 与最大每日配额比较，计算剩余配额
- 返回结构
  - should、reason、remaining，供主流程决策

```mermaid
flowchart TD
W["isInPublishWindow()"] --> TIME{"当前时间是否在窗口内？"}
TIME --> |否| OUT["返回不可发布"]
TIME --> |是| COUNT["getTodayArticleCount()"]
COUNT --> QUOTA{"是否超过每日配额？"}
QUOTA --> |是| LIMIT["返回不可发布已达配额"]
QUOTA --> |否| OK["返回可发布remaining>0"]
```

**图表来源**
- [scripts/news-auto/scheduler.ts:29-60](file://scripts/news-auto/scheduler.ts#L29-L60)
- [scripts/news-auto/scheduler.ts:67-103](file://scripts/news-auto/scheduler.ts#L67-L103)

**章节来源**
- [scripts/news-auto/scheduler.ts:29-103](file://scripts/news-auto/scheduler.ts#L29-L103)

### 配置中心（config.ts）
- 发布策略：最大每日文章数、发布时间点、自动发布开关
- 关键词过滤：必需词、可选词、排除词
- AI参数：模型、最大Token、温度
- **新增内容质量阈值**：最小/最大字数、关键词密度
- 目标语言：zh/en/id/th/vi/ar
- 分类映射：用于新闻源分类到文章分类的映射

**章节来源**
- [scripts/news-auto/config.ts:6-79](file://scripts/news-auto/config.ts#L6-L79)

### 新闻源配置（news-sources.config.ts）
- 结构定义：名称、URL、类型（rss/web/rss+web）、RSS地址、CSS选择器、分类、语言、优先级、启用状态、备注、自定义headers
- 工具函数：获取启用源、按分类/语言筛选
- 维护建议：新增源在数组中添加对象，停用设为disabled，调整优先级以控制抓取顺序

**章节来源**
- [scripts/news-auto/news-sources.config.ts:17-178](file://scripts/news-auto/news-sources.config.ts#L17-L178)

### 安全验证机制（API路由）
**更新** 系统现在采用条件验证机制，只有在配置了 CRON_SECRET 时才进行严格验证，未配置时记录警告但允许执行。

- **条件验证逻辑**
  - 当 CRON_SECRET 环境变量存在时：严格验证请求头 `Authorization: Bearer {secret}`
  - 当 CRON_SECRET 环境变量不存在时：记录警告日志但继续执行自动化流程
- **改进的日志记录**
  - 未配置 CRON_SECRET 时输出 `⚠️ WARNING: CRON_SECRET not configured`
  - 配置正确时正常执行，无额外日志
- **错误处理增强**
  - API密钥检查：确保 DASHSCOPE_API_KEY 已配置
  - 详细的错误响应：包含错误类型和消息
  - 统一的500错误处理，便于监控和调试

```mermaid
flowchart TD
REQ["收到请求"] --> SECRET{"检查CRON_SECRET"}
SECRET --> |存在| AUTH{"验证Authorization头"}
AUTH --> |匹配| EXEC["执行自动化流程"]
AUTH --> |不匹配| UNAUTH["返回401 Unauthorized"]
SECRET --> |不存在| WARN["记录警告日志"]
WARN --> EXEC
EXEC --> CHECKAPI{"检查DASHSCOPE_API_KEY"}
CHECKAPI --> |存在| DONE["返回成功响应"]
CHECKAPI --> |不存在| ERR500["返回500错误"]
```

**图表来源**
- [app/api/cron/news/route.ts:5-58](file://app/api/cron/news/route.ts#L5-L58)

**章节来源**
- [app/api/cron/news/route.ts:5-58](file://app/api/cron/news/route.ts#L5-L58)

### 测试脚本（新增）
系统现在包含多个测试脚本以支持开发和调试：

- **完整工作流测试**：`test-news-workflow.ts` - 测试从抓取到AI处理的完整流程
- **AI图像生成测试**：`test-ai-image.ts` - 详细的AI图像生成测试，包含成功率统计
- **快速AI图像测试**：`test-ai-image-quick.ts` - 快速测试AI图像生成功能

**章节来源**
- [scripts/test-news-workflow.ts:1-68](file://scripts/test-news-workflow.ts#L1-L68)
- [scripts/test-ai-image.ts:1-142](file://scripts/test-ai-image.ts#L1-L142)
- [scripts/test-ai-image-quick.ts:1-94](file://scripts/test-ai-image-quick.ts#L1-L94)

## 依赖关系分析
- 外部依赖
  - rss-parser：RSS解析
  - axios/cheerio：HTTP请求与HTML解析
  - @sanity/client：Sanity写入
  - dotenv：环境变量加载
- 内部耦合
  - 主流程依赖调度器、抓取器、AI处理器、发布器
  - 抓取器依赖新闻源配置与全局配置
  - 发布器依赖Sanity客户端与文章Schema
  - API路由依赖主流程并进行安全校验

```mermaid
graph LR
PKG["package.json 依赖"] --> AX["axios"]
PKG --> RP["rss-parser"]
PKG --> CH["cheerio"]
PKG --> SAN["@sanity/client"]
IDX["index.ts"] --> SCH["scheduler.ts"]
IDX --> CRW["crawler.ts"]
IDX --> AI["ai-processor.ts"]
IDX --> PUB["publisher.ts"]
CRW --> SRC["news-sources.config.ts"]
CRW --> CFG["config.ts"]
AI --> CFG
AI --> IDX
PUB --> SAN["lib/sanity/client.ts"]
SAN --> ART["sanity/schemas/article.ts"]
API["app/api/cron/news/route.ts"] --> IDX
```

**图表来源**
- [package.json:13-29](file://package.json#L13-L29)
- [scripts/news-auto/index.ts:1-7](file://scripts/news-auto/index.ts#L1-L7)
- [scripts/news-auto/crawler.ts:1-5](file://scripts/news-auto/crawler.ts#L1-L5)
- [scripts/news-auto/ai-processor.ts:1-3](file://scripts/news-auto/ai-processor.ts#L1-L3)
- [scripts/news-auto/publisher.ts:1](file://scripts/news-auto/publisher.ts#L1)
- [lib/sanity/client.ts:1-18](file://lib/sanity/client.ts#L1-L18)
- [sanity/schemas/article.ts:1-2](file://sanity/schemas/article.ts#L1-L2)
- [app/api/cron/news/route.ts:2](file://app/api/cron/news/route.ts#L2)

**章节来源**
- [package.json:13-29](file://package.json#L13-L29)

## 性能考量
- 抓取阶段
  - 控制每源抓取数量（截断前N条），降低后续处理压力
  - 去重与关键词过滤减少无效内容
- AI处理阶段
  - 为避免API限流，处理间隔增加延时
  - 多语言翻译失败时快速降级，保证整体吞吐
  - **AI图像生成采用异步任务和轮询机制，避免长时间阻塞**
- 发布阶段
  - 图片下载与上传为IO密集型，注意网络稳定性
  - 批量发布时加入延时，避免写入限流
  - **重复检测采用双重检查（URL + 标题），提高准确性**
- 配置优化
  - 合理设置每日配额与发布时间点，避免高峰期拥堵
  - 优先级高的源先抓取，提升命中率

## 故障排除指南
- API密钥缺失
  - 现象：AI处理报错提示缺少API Key
  - 处理：确保环境变量DASHSCOPE_API_KEY已配置
- 定时任务未触发
  - 现象：API返回未授权或流程未执行
  - 处理：确认CRON_SECRET配置正确，且请求头匹配
  - **新增**：如果未配置CRON_SECRET，系统会记录警告但继续执行
- 发布失败
  - 现象：分类ID为空、图片上传失败、重复文章
  - 处理：检查文章分类是否存在、图片URL可访问、中文标题唯一性
- 时间窗口不生效
  - 现象：Vercel Hobby套餐时间浮动导致未在窗口内
  - 处理：接受±90分钟窗口；本地测试可开启绕过时间检查
- 新闻源不可用
  - 现象：RSS解析失败或网页抓取无结果
  - 处理：检查RSS地址、选择器有效性、必要时添加自定义headers伪装UA
- **AI图像生成失败**
  - 现象：AI图像生成接口调用失败或超时
  - 处理：检查DASHSCOPE_API_KEY配置，确认网络连接正常，查看日志中的错误信息
- **重复检测误判**
  - 现象：相同文章被重复发布
  - 处理：检查URL是否发生变化，确认重复检测逻辑（URL + 标题双重检查）
- **安全验证问题**
  - 现象：API返回401错误或流程被阻止
  - 处理：检查CRON_SECRET配置，确保Authorization头格式正确
- **测试脚本问题**
  - 现象：测试脚本无法运行或测试失败
  - 处理：确保环境变量正确配置，检查API密钥有效性，查看详细的测试报告

**章节来源**
- [app/api/cron/news/route.ts:20-26](file://app/api/cron/news/route.ts#L20-L26)
- [scripts/news-auto/ai-processor.ts:22-24](file://scripts/news-auto/ai-processor.ts#L22-L24)
- [scripts/news-auto/publisher.ts:14-18](file://scripts/news-auto/publisher.ts#L14-L18)
- [scripts/news-auto/scheduler.ts:29-60](file://scripts/news-auto/scheduler.ts#L29-L60)
- [scripts/news-auto/crawler.ts:26-29](file://scripts/news-auto/crawler.ts#L26-L29)

## 结论
本系统通过清晰的模块划分与独立配置，实现了从新闻采集、AI智能处理到自动发布的完整闭环。其设计具备良好的可维护性与扩展性：新闻源配置独立于抓取逻辑，调度策略与AI参数集中管理，发布流程与内容模型解耦。**新增的AI图像生成功能进一步提升了内容的视觉表现力，改进的重复检测机制提高了系统的准确性，增强的安全验证机制提供了更灵活的部署选项，改进的错误消息和警告日志增强了系统的可观测性和易维护性，新增的测试脚本为开发和调试提供了更好的支持。**结合定时任务的安全校验与完善的错误处理，能够在生产环境中稳定运行。

## 附录

### 配置示例与使用指南
- 环境变量
  - DASHSCOPE_API_KEY：通义千问API密钥
  - SANITY_API_TOKEN：Sanity写入令牌
  - CRON_SECRET：定时任务访问密钥（可选，未配置时系统记录警告但继续执行）
  - NEWS_BYPASS_TIME_CHECK：本地测试时绕过时间窗口检查（可选）
- 定时任务配置（Vercel Cron）
  - 在部署平台配置定时触发，指向 /api/cron/news
  - 请求头需携带 Authorization: Bearer {CRON_SECRET}
  - **可选配置**：如果未设置CRON_SECRET，系统会记录警告但继续执行
- 新闻源维护
  - 在 news-sources.config.ts 中新增/停用/调整优先级
  - 仅RSS：type='rss' 并填写 rss
  - 仅网页：type='web' 并填写 selector
  - RSS+网页：type='rss+web'，优先RSS，失败则回退网页
- 关键词过滤
  - required：至少包含一个
  - exclude：全部排除
  - optional：辅助筛选（不影响发布）
- AI参数
  - model：qwen-turbo
  - maxTokens：控制输出长度
  - temperature：控制创造性
- **新增内容质量阈值**
  - minWordCount：最小字数（500）
  - maxWordCount：最大字数（2000）
  - minKeywordDensity：最小关键词密度（0.01）
- 目标语言
  - zh/en/id/th/vi/ar

**章节来源**
- [scripts/news-auto/config.ts:6-79](file://scripts/news-auto/config.ts#L6-L79)
- [scripts/news-auto/news-sources.config.ts:46-178](file://scripts/news-auto/news-sources.config.ts#L46-L178)
- [app/api/cron/news/route.ts:7-15](file://app/api/cron/news/route.ts#L7-L15)

### 数据模型概览（Sanity）
- 文章Schema支持6种语言的标题、摘要与正文
- 分类为引用类型，关联文章分类
- SEO字段包含多语言metaTitle与metaDescription
- 发布状态、发布时间、来源信息（含是否AI生成）
- **新增封面图字段，支持AI生成的配图**

**章节来源**
- [sanity/schemas/article.ts:8-247](file://sanity/schemas/article.ts#L8-L247)

### 测试与验证
- 快速测试RSS抓取：使用 test-news.js 验证RSS可用性与关键词匹配
- 本地开发：可临时设置绕过时间检查以便调试
- **AI图像生成测试**：可通过直接调用 generateAIImage 函数测试图像生成功能
- **安全验证测试**：可测试CRON_SECRET配置与未配置两种场景的行为差异
- **完整工作流测试**：`test-news-workflow.ts` - 测试从抓取到发布的完整流程
- **AI图像生成详细测试**：`test-ai-image.ts` - 包含成功率统计和详细报告
- **快速AI图像测试**：`test-ai-image-quick.ts` - 简化的AI图像生成测试

**章节来源**
- [scripts/test-news.js:1-40](file://scripts/test-news.js#L1-L40)
- [scripts/test-news-workflow.ts:1-68](file://scripts/test-news-workflow.ts#L1-L68)
- [scripts/test-ai-image.ts:1-142](file://scripts/test-ai-image.ts#L1-L142)
- [scripts/test-ai-image-quick.ts:1-94](file://scripts/test-ai-image-quick.ts#L1-L94)

### **新增功能详解**

#### AI图像生成功能
系统现已集成通义万相AI图像生成功能，为每篇文章自动生成专业配图：
- **图像生成流程**：基于文章内容生成英文图像描述prompt，调用通义万相API生成图像
- **异步处理**：采用异步任务提交和轮询机制，最长等待60秒
- **降级策略**：若AI生成失败但原文有图片，则使用原图；若两者都无，则记录警告
- **质量控制**：生成1024×768像素的专业科技风格图片

#### 改进的重复检测机制
发布流程中的重复检测现已升级为双重检查：
- **URL优先检测**：首先检查原始URL是否已存在，这是最准确的重复判断
- **标题补充检测**：若URL检测未发现重复，则检查中文标题是否重复
- **日志记录**：详细记录重复检测过程和结果，便于调试和监控

#### 增强的内容处理工作流
AI处理器现在包含完整的端到端处理流程：
- **内容改写**：生成专业中文行业资讯
- **多语言翻译**：支持6种语言的标题、摘要和正文翻译
- **关键词提取**：提取英文SEO关键词
- **AI图像生成**：为每篇文章生成专业配图
- **SEO优化**：生成多语言metaTitle与metaDescription

#### 改进的安全验证机制
**更新** 系统的安全验证机制现已支持条件验证模式：
- **条件验证逻辑**：只有在配置了 CRON_SECRET 时才进行严格验证
- **灵活部署支持**：未配置CRON_SECRET时记录警告但继续执行，便于开发和测试环境
- **增强的错误处理**：提供更清晰的错误消息和日志记录
- **向后兼容**：现有生产环境的严格验证模式保持不变

#### 新增测试脚本
系统现在包含多个专门的测试脚本：
- **完整工作流测试**：`test-news-workflow.ts` - 测试从抓取到发布的完整流程
- **AI图像生成测试**：`test-ai-image.ts` - 详细的AI图像生成测试，包含成功率统计
- **快速AI图像测试**：`test-ai-image-quick.ts` - 简化的AI图像生成测试

这些测试脚本提供了：
- **详细的成功率统计**：包括总测试数、成功数、失败数和平均耗时
- **错误诊断**：详细的错误信息和建议
- **性能监控**：测试结果可用于监控API性能变化
- **开发调试**：帮助开发者快速定位和解决问题

**章节来源**
- [scripts/news-auto/ai-processor.ts:156-354](file://scripts/news-auto/ai-processor.ts#L156-L354)
- [scripts/news-auto/publisher.ts:14-35](file://scripts/news-auto/publisher.ts#L14-L35)
- [scripts/news-auto/config.ts:28-34](file://scripts/news-auto/config.ts#L28-L34)
- [app/api/cron/news/route.ts:5-58](file://app/api/cron/news/route.ts#L5-L58)
- [scripts/test-news-workflow.ts:1-68](file://scripts/test-news-workflow.ts#L1-L68)
- [scripts/test-ai-image.ts:1-142](file://scripts/test-ai-image.ts#L1-L142)
- [scripts/test-ai-image-quick.ts:1-94](file://scripts/test-ai-image-quick.ts#L1-L94)