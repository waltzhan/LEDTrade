# 用户界面组件

<cite>
**本文引用的文件**
- [components/layout/navbar.tsx](file://components/layout/navbar.tsx)
- [components/layout/footer.tsx](file://components/layout/footer.tsx)
- [components/forms/InquiryForm.tsx](file://components/forms/InquiryForm.tsx)
- [components/ui/breadcrumb.tsx](file://components/ui/breadcrumb.tsx)
- [components/ui/pagination.tsx](file://components/ui/pagination.tsx)
- [components/analytics/GoogleAnalytics.tsx](file://components/analytics/GoogleAnalytics.tsx)
- [components/products/ai-citability-block.tsx](file://components/products/ai-citability-block.tsx)
- [lib/i18n/config.ts](file://lib/i18n/config.ts)
- [lib/geo/ai-citability.ts](file://lib/geo/ai-citability.ts)
- [messages/en.json](file://messages/en.json)
- [tailwind.config.js](file://tailwind.config.js)
- [app/[locale]/page.tsx](file://app/[locale]/page.tsx)
- [app/[locale]/contact/page.tsx](file://app/[locale]/contact/page.tsx)
- [app/[locale]/products/page.tsx](file://app/[locale]/products/page.tsx)
- [app/[locale]/products/[slug]/page.tsx](file://app/[locale]/products/[slug]/page.tsx)
- [app/[locale]/glossary/page.tsx](file://app/[locale]/glossary/page.tsx)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [组件详解](#组件详解)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件为 GoPro Trade 网站的用户界面组件综合文档，聚焦于可复用组件库的组织与职责、组件间通信与状态管理策略、导航栏与页脚的国际化与响应式实现、表单组件（尤其是询盘表单）的校验与提交流程、UI 工具组件（面包屑、分页）的实现与定制、以及样式系统（Tailwind CSS）与主题、响应式断点的使用规范。文档同时提供组件使用示例与最佳实践，帮助开发者与设计师高效协作。

**更新** 新增AI可引用内容块组件，用于展示优化后的产品技术规格和比较表格，提升AI搜索引擎的可理解性和引用能力。

## 项目结构
组件层位于 components 目录下，按功能域划分：
- layout：布局级组件（导航栏、页脚）
- ui：通用 UI 工具组件（面包屑、分页）
- forms：业务表单组件（询盘表单）
- products：产品相关组件（AI可引用内容块）
- analytics：分析埋点集成（Google Analytics）

页面层位于 app/[locale] 下，采用多语言路由与静态参数生成，页面内组合使用上述组件完成完整页面体验。

```mermaid
graph TB
subgraph "页面层"
P1["app/[locale]/page.tsx"]
P2["app/[locale]/contact/page.tsx"]
P3["app/[locale]/products/page.tsx"]
P4["app/[locale]/products/[slug]/page.tsx"]
P5["app/[locale]/glossary/page.tsx"]
end
subgraph "布局组件"
N["components/layout/navbar.tsx"]
F["components/layout/footer.tsx"]
end
subgraph "UI工具"
B["components/ui/breadcrumb.tsx"]
Pg["components/ui/pagination.tsx"]
end
subgraph "表单"
IF["components/forms/InquiryForm.tsx"]
end
subgraph "产品组件"
ACB["components/products/ai-citability-block.tsx"]
end
subgraph "分析"
GA["components/analytics/GoogleAnalytics.tsx"]
end
subgraph "AI优化"
AIC["lib/geo/ai-citability.ts"]
end
subgraph "国际化"
I18N["lib/i18n/config.ts"]
MSG["messages/en.json"]
end
subgraph "样式"
TW["tailwind.config.js"]
end
P1 --> N
P1 --> F
P2 --> N
P2 --> F
P2 --> IF
P3 --> N
P3 --> F
P3 --> B
P4 --> N
P4 --> F
P4 --> ACB
P5 --> ACB
N --> I18N
F --> I18N
IF --> GA
N --> MSG
F --> MSG
ACB --> AIC
B --> TW
Pg --> TW
```

**图表来源**
- [app/[locale]/page.tsx](file://app/[locale]/page.tsx#L1-L334)
- [app/[locale]/contact/page.tsx](file://app/[locale]/contact/page.tsx#L1-L227)
- [app/[locale]/products/page.tsx](file://app/[locale]/products/page.tsx#L1-L295)
- [app/[locale]/products/[slug]/page.tsx](file://app/[locale]/products/[slug]/page.tsx#L1-L467)
- [app/[locale]/glossary/page.tsx](file://app/[locale]/glossary/page.tsx#L1-L167)
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)
- [components/forms/InquiryForm.tsx:1-298](file://components/forms/InquiryForm.tsx#L1-L298)
- [components/ui/breadcrumb.tsx:1-87](file://components/ui/breadcrumb.tsx#L1-L87)
- [components/ui/pagination.tsx:1-83](file://components/ui/pagination.tsx#L1-L83)
- [components/analytics/GoogleAnalytics.tsx:1-93](file://components/analytics/GoogleAnalytics.tsx#L1-L93)
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)
- [lib/geo/ai-citability.ts:1-196](file://lib/geo/ai-citability.ts#L1-L196)
- [lib/i18n/config.ts:1-16](file://lib/i18n/config.ts#L1-L16)
- [messages/en.json:1-200](file://messages/en.json#L1-L200)
- [tailwind.config.js:1-18](file://tailwind.config.js#L1-L18)

**章节来源**
- [app/[locale]/page.tsx](file://app/[locale]/page.tsx#L1-L334)
- [app/[locale]/contact/page.tsx](file://app/[locale]/contact/page.tsx#L1-L227)
- [app/[locale]/products/page.tsx](file://app/[locale]/products/page.tsx#L1-L295)
- [app/[locale]/products/[slug]/page.tsx](file://app/[locale]/products/[slug]/page.tsx#L1-L467)
- [app/[locale]/glossary/page.tsx](file://app/[locale]/glossary/page.tsx#L1-L167)

## 核心组件
- 导航栏组件：负责站点主导航、语言切换、移动端菜单、CTA 按钮与当前路径高亮。
- 页脚组件：提供公司信息、快速链接、联系方式、社交图标、版权与法律条款。
- 询盘表单：收集公司、联系人、邮箱、电话、国家、感兴趣产品、数量范围、备注，并提交到后端 API。
- 面包屑组件：生成结构化数据与可访问的导航序列。
- 分页组件：支持页码省略与 RTL 布局适配。
- **AI可引用内容块组件**：展示AI优化的产品技术规格、比较表格和FAQ，提升搜索引擎可理解性。

**更新** 新增AI可引用内容块组件，包含问答内容块、技术规格对比表和FAQ列表组件。

**章节来源**
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)
- [components/forms/InquiryForm.tsx:1-298](file://components/forms/InquiryForm.tsx#L1-L298)
- [components/ui/breadcrumb.tsx:1-87](file://components/ui/breadcrumb.tsx#L1-L87)
- [components/ui/pagination.tsx:1-83](file://components/ui/pagination.tsx#L1-L83)
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)

## 架构总览
组件间通信与状态管理策略：
- 组件间通信：通过 props 向子组件传递消息与国际化键值；页面层负责聚合翻译与数据，再注入到组件。
- 状态管理：导航栏与表单使用 React 本地状态；面包屑与分页为纯展示组件，无内部状态；AI内容块组件使用本地展开/收起状态。
- 国际化：统一从 lib/i18n/config.ts 读取可用语言与方向性；页面层按 locale 选择对应 messages 文件。
- 分析埋点：询盘表单提交成功后触发 GA4 自定义事件，页面切换通过 GoogleAnalytics 组件自动上报。
- **AI优化集成**：产品页面通过AI优化函数生成结构化内容，传递给AI可引用内容块组件进行展示。

**更新** 新增AI优化内容生成和集成流程。

```mermaid
sequenceDiagram
participant U as "用户"
participant PF as "页面组件"
participant ACB as "AI可引用内容块"
participant AIC as "AI优化函数"
U->>PF : 访问产品页面
PF->>AIC : 生成AI优化内容
AIC-->>PF : 返回结构化内容
PF->>ACB : 注入问答内容、数据点、关键词
U->>ACB : 点击展开/收起
ACB-->>U : 切换内容显示状态
```

**图表来源**
- [app/[locale]/products/[slug]/page.tsx:218-227](file://app/[locale]/products/[slug]/page.tsx#L218-L227)
- [lib/geo/ai-citability.ts:22-53](file://lib/geo/ai-citability.ts#L22-L53)
- [components/products/ai-citability-block.tsx:21-90](file://components/products/ai-citability-block.tsx#L21-L90)

## 组件详解

### 导航栏组件（Navbar）
职责与特性：
- 多语言支持：基于 locale 生成本地化链接与语言名称映射，支持 cookie 记忆语言偏好。
- 响应式设计：桌面端水平导航与语言切换下拉，移动端三线按钮展开菜单，含移动端语言切换与 CTA。
- 当前路径高亮：根据去除 locale 的路径进行高亮匹配。
- RTL 适配：根据语言方向动态添加类名以影响布局。

交互流程（语言切换）：
```mermaid
flowchart TD
Start(["点击语言按钮"]) --> Open["展开语言下拉菜单"]
Open --> Select["选择目标语言"]
Select --> SetCookie["设置 cookie: NEXT_LOCALE=新语言"]
SetCookie --> Close["关闭下拉菜单"]
Close --> Navigate["根据当前路径生成带locale的href并跳转"]
```

**图表来源**
- [components/layout/navbar.tsx:36-59](file://components/layout/navbar.tsx#L36-L59)
- [lib/i18n/config.ts:1-16](file://lib/i18n/config.ts#L1-L16)

**章节来源**
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [lib/i18n/config.ts:1-16](file://lib/i18n/config.ts#L1-L16)

### 页脚组件（Footer）
职责与特性：
- 公司信息与描述：品牌标识与企业简介。
- 快速导航：首页、产品、方案、关于、支持等常用链接。
- 联系方式：地址与邮箱展示，支持多语言地址值切换。
- 社交媒体：占位链接与 SVG 图标。
- 版权与法律条款：隐私政策与使用条款链接。
- RTL 适配：针对特定语言（如阿拉伯语）调整布局方向。

**章节来源**
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)

### 询盘表单（InquiryForm）
职责与特性：
- 字段覆盖：公司名称、联系人、邮箱、电话、国家、感兴趣产品（多选）、数量范围、备注。
- 提交流程：阻止默认提交，构造请求体，调用 /api/inquiry，依据响应更新状态。
- 成功与错误反馈：显示绿色/红色提示框，成功后重置表单。
- GA4 转化跟踪：成功提交后触发自定义事件，上报 locale、产品数量、数量范围、国家等维度。
- 国际化：所有文案来自传入的消息对象，支持多语言占位符与选项文案。

提交流程时序：
```mermaid
sequenceDiagram
participant U as "用户"
participant IF as "InquiryForm"
participant API as "/api/inquiry"
participant GA as "trackEvent"
U->>IF : 填写并点击提交
IF->>IF : 阻止默认提交/设置提交中状态
IF->>API : POST 请求含表单数据与locale
API-->>IF : 返回响应
alt 成功
IF->>GA : 发送自定义事件
IF->>IF : 显示成功提示/重置表单
else 失败或异常
IF->>IF : 显示错误提示
end
```

**图表来源**
- [components/forms/InquiryForm.tsx:73-117](file://components/forms/InquiryForm.tsx#L73-L117)
- [components/analytics/GoogleAnalytics.tsx:78-84](file://components/analytics/GoogleAnalytics.tsx#L78-L84)

**章节来源**
- [components/forms/InquiryForm.tsx:1-298](file://components/forms/InquiryForm.tsx#L1-L298)
- [components/analytics/GoogleAnalytics.tsx:1-93](file://components/analytics/GoogleAnalytics.tsx#L1-L93)

### 面包屑组件（Breadcrumb）
职责与特性：
- 结构化数据：自动生成 JSON-LD BreadcrumbList，提升 SEO。
- 可访问性：最后一项标注当前页，其余项为可点击链接。
- RTL 支持：通过容器类名控制布局方向。
- 动态链接：基于站点基址与传入项生成完整 URL。

实现要点：
- 生成 BreadcrumbList 结构化数据脚本节点。
- 渲染可见的面包屑导航，最后一个项无链接。
- 支持 RTL 布局与可访问属性。

**章节来源**
- [components/ui/breadcrumb.tsx:1-87](file://components/ui/breadcrumb.tsx#L1-L87)

### 分页组件（Pagination）
职责与特性：
- 简化分页逻辑：当总页数不超过 7 时直接显示全部页码；否则在首尾与当前页周围省略中间页码。
- URL 构造：根据基础 URL 与查询参数拼接页码链接，自动处理已有查询串的连接符。
- RTL 适配：通过容器类名反转箭头方向，配合布局镜像。
- 禁用状态：当前页为第一页或最后一页时禁用上一页/下一页按钮。

**章节来源**
- [components/ui/pagination.tsx:1-83](file://components/ui/pagination.tsx#L1-L83)

### AI可引用内容块组件（AICitableContentBlock）
职责与特性：
- **问答内容块**：展示AI优化的问题-答案结构，支持展开/收起，包含数据点列表和关键词标签。
- **技术规格对比表**：提供两个产品或规格的对比分析，支持AI友好格式。
- **FAQ列表**：展示常见问题及其答案，支持逐项展开查看。
- **交互设计**：点击标题或按钮触发展开/收起，使用动画过渡效果。
- **SEO优化**：使用语义化HTML标签，提升搜索引擎可理解性。

实现要点：
- 使用React状态管理展开/收起状态。
- 支持数据点列表的条件渲染。
- 关键词标签使用圆角样式，支持悬停效果。
- FAQ列表使用accordion设计，支持单个条目展开。

**新增** 完整的AI可引用内容块组件，包含问答展示、数据点列表、关键词标签和FAQ功能。

**章节来源**
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)

### AI优化内容生成器（AICitability Generator）
职责与特性：
- **产品描述优化**：基于产品名称、类别、特性、规格和应用场景生成AI可引用的描述内容。
- **技术定义优化**：为LED术语生成结构化的定义内容，包含上下文和示例。
- **对比分析优化**：生成产品或技术对比的结构化内容。
- **最佳实践优化**：提供行业最佳实践的结构化内容。
- **统计数据优化**：整理行业统计数据的结构化内容。
- **FAQ生成**：为产品生成常见问题列表。

实现要点：
- 遵循GEO（Generative Engine Optimization）最佳实践。
- 控制内容长度在134-167词之间。
- 生成结构化的问答对、数据点和关键词。
- 支持多种内容类型的生成器函数。

**新增** AI优化内容生成器，为AI可引用内容块提供数据支持。

**章节来源**
- [lib/geo/ai-citability.ts:1-196](file://lib/geo/ai-citability.ts#L1-L196)

### 页面中的组件使用示例
- 首页（[app/[locale]/page.tsx](file://app/[locale]/page.tsx#L1-L334)）：在页面中组合导航栏、页脚、结构化数据与内容区块。
- 产品页（[app/[locale]/products/page.tsx](file://app/[locale]/products/page.tsx#L1-L295)）：引入面包屑组件，构建分类侧边栏与产品网格。
- **产品详情页**（[app/[locale]/products/[slug]/page.tsx](file://app/[locale]/products/[slug]/page.tsx#L1-L467)）：使用AI可引用内容块展示优化后的产品描述和FAQ。
- **术语表页面**（[app/[locale]/glossary/page.tsx](file://app/[locale]/glossary/page.tsx#L1-L167)）：批量展示多个AI优化的术语定义。
- 询盘页（[app/[locale]/contact/page.tsx](file://app/[locale]/contact/page.tsx#L1-L227)）：注入表单所需消息与产品选项，右侧展示联系信息与快捷链接。

**更新** 新增产品详情页和术语表页面中AI可引用内容块的使用示例。

**章节来源**
- [app/[locale]/page.tsx](file://app/[locale]/page.tsx#L1-L334)
- [app/[locale]/products/page.tsx](file://app/[locale]/products/page.tsx#L1-L295)
- [app/[locale]/products/[slug]/page.tsx](file://app/[locale]/products/[slug]/page.tsx#L1-L467)
- [app/[locale]/glossary/page.tsx](file://app/[locale]/glossary/page.tsx#L1-L167)
- [app/[locale]/contact/page.tsx](file://app/[locale]/contact/page.tsx#L1-L227)

## 依赖关系分析
- 组件依赖国际化配置：Navbar 与 Footer 依赖 lib/i18n/config.ts 的语言列表与方向性。
- 页面依赖消息文件：各页面按 locale 选择 messages 文件，向组件注入文案。
- 表单依赖分析埋点：InquiryForm 依赖 GoogleAnalytics 的 trackEvent 工具。
- UI 组件依赖 Tailwind：面包屑与分页使用 Tailwind 类名与主题颜色。
- **AI组件依赖优化函数**：AI可引用内容块依赖AI优化生成器函数。
- **页面依赖AI内容**：产品详情页和术语表页面依赖AI优化内容生成。

**更新** 新增AI组件依赖关系分析。

```mermaid
graph LR
I18N["lib/i18n/config.ts"] --> NAV["Navbar"]
I18N --> FOOT["Footer"]
MSG["messages/*.json"] --> NAV
MSG --> FOOT
PAGE["页面组件"] --> NAV
PAGE --> FOOT
PAGE --> IF["InquiryForm"]
PAGE --> ACB["AI可引用内容块"]
ACB --> AIC["AI优化生成器"]
IF --> GA["GoogleAnalytics"]
UI["UI组件"] --> TW["tailwind.config.js"]
```

**图表来源**
- [lib/i18n/config.ts:1-16](file://lib/i18n/config.ts#L1-L16)
- [messages/en.json:1-200](file://messages/en.json#L1-L200)
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)
- [components/forms/InquiryForm.tsx:1-298](file://components/forms/InquiryForm.tsx#L1-L298)
- [components/analytics/GoogleAnalytics.tsx:1-93](file://components/analytics/GoogleAnalytics.tsx#L1-L93)
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)
- [lib/geo/ai-citability.ts:1-196](file://lib/geo/ai-citability.ts#L1-L196)
- [tailwind.config.js:1-18](file://tailwind.config.js#L1-L18)

**章节来源**
- [lib/i18n/config.ts:1-16](file://lib/i18n/config.ts#L1-L16)
- [messages/en.json:1-200](file://messages/en.json#L1-L200)
- [tailwind.config.js:1-18](file://tailwind.config.js#L1-L18)

## 性能考量
- 图片优化：产品页对首屏图片使用优先加载策略，其余图片懒加载，合理设置 sizes 与填充容器，减少主线程阻塞。
- 组件懒加载：页面按需加载，避免不必要的初始渲染。
- 分页省略：分页组件在页数较多时省略中间页码，降低 DOM 节点数量。
- 分析脚本策略：Google Analytics 使用 afterInteractive 策略，避免阻塞首包渲染。
- **AI内容延迟加载**：AI可引用内容块在用户交互时才展开，减少初始渲染负担。
- **内容生成缓存**：AI优化内容生成器支持缓存机制，避免重复计算。

**更新** 新增AI内容相关的性能考量。

**章节来源**
- [app/[locale]/products/page.tsx](file://app/[locale]/products/page.tsx#L230-L304)
- [components/analytics/GoogleAnalytics.tsx:43-67](file://components/analytics/GoogleAnalytics.tsx#L43-L67)
- [components/products/ai-citability-block.tsx:21-90](file://components/products/ai-citability-block.tsx#L21-L90)

## 故障排查指南
- 语言切换无效：检查语言 cookie 设置与 getLocalizedHref 生成的链接是否正确，确认 locale 是否在可用列表中。
- 询盘提交失败：查看网络面板与后端返回状态，确认 /api/inquiry 接口可达；前端会显示错误提示并保持表单状态。
- 面包屑不生效：确认结构化数据脚本已注入，且 items 数组包含至少两项；检查最后一项是否为当前页。
- 分页链接异常：检查 baseUrl 是否包含查询参数，确保连接符拼接正确；RTL 模式下箭头方向是否反转。
- 分析埋点未上报：确认 NEXT_PUBLIC_GA_ID 环境变量存在，且页面已引入 GoogleAnalytics 组件。
- **AI内容不显示**：检查AI优化函数是否正确生成内容，确认页面是否正确调用AI可引用内容块组件。
- **FAQ展开异常**：检查FAQ组件的状态管理逻辑，确认展开/收起状态切换正常。

**更新** 新增AI相关内容相关的故障排查指南。

**章节来源**
- [components/layout/navbar.tsx:36-59](file://components/layout/navbar.tsx#L36-L59)
- [components/forms/InquiryForm.tsx:73-117](file://components/forms/InquiryForm.tsx#L73-L117)
- [components/ui/breadcrumb.tsx:24-42](file://components/ui/breadcrumb.tsx#L24-L42)
- [components/ui/pagination.tsx:13-16](file://components/ui/pagination.tsx#L13-L16)
- [components/analytics/GoogleAnalytics.tsx:37-67](file://components/analytics/GoogleAnalytics.tsx#L37-L67)
- [components/products/ai-citability-block.tsx:142-177](file://components/products/ai-citability-block.tsx#L142-L177)

## 结论
该组件体系以清晰的职责分离与可复用性为核心，结合国际化与响应式设计，提供了良好的用户体验与可维护性。导航栏与页脚承担统一的品牌与导航职责，表单与 UI 工具组件分别聚焦业务与通用展示，分析埋点贯穿关键转化路径。**新增的AI可引用内容块组件显著提升了内容的搜索引擎可理解性和AI引用能力，为网站在AI时代提供了竞争优势。** 建议在后续迭代中持续关注性能指标与可访问性，进一步完善 RTL 与多语言的边界场景，同时优化AI内容生成的质量和效率。

**更新** 总结AI可引用内容块组件的重要价值和未来发展方向。

## 附录

### 组件样式系统与主题
- Tailwind 配置：content 覆盖 app 与 components 目录，确保样式扫描与按需生成。
- 主题扩展：新增 gopro-blue 与 gopro-cyan 两色，用于品牌一致性的色彩体系。
- 响应式断点：组件广泛使用 sm/md/lg/xl 等断点类名，确保在不同设备上的良好表现。
- **AI组件样式**：AI可引用内容块使用蓝色边框强调，数据点列表使用浅蓝色背景，关键词标签使用灰色背景。

**更新** 新增AI组件相关的样式说明。

**章节来源**
- [tailwind.config.js:1-18](file://tailwind.config.js#L1-L18)
- [components/products/ai-citability-block.tsx:25-74](file://components/products/ai-citability-block.tsx#L25-L74)

### 组件使用最佳实践
- 导航栏
  - 保持语言切换与路径高亮的一致性，避免重复渲染。
  - 移动端菜单与桌面端菜单的状态独立管理，避免相互干扰。
- 页脚
  - 快速链接与法律条款保持与导航栏一致的命名与顺序。
  - 社交链接占位符需在上线前替换为真实链接。
- 询盘表单
  - 表单字段必填与类型校验需与后端接口一致，避免无效提交。
  - 成功提交后及时重置表单并清空多选状态。
- 面包屑与分页
  - 面包屑最后一项不生成链接，避免重复路径。
  - 分页在 RTL 模式下需同步调整布局与箭头方向。
- **AI可引用内容块**
  - 确保AI优化内容的准确性，定期更新技术规格和应用场景。
  - 合理控制内容长度，遵循GEO最佳实践的词数限制。
  - FAQ内容应覆盖用户最关心的问题，保持答案的简洁明了。
  - 数据点列表应包含关键的技术参数和认证信息。

**更新** 新增AI可引用内容块组件的最佳实践指南。

**章节来源**
- [components/products/ai-citability-block.tsx:21-178](file://components/products/ai-citability-block.tsx#L1-L178)
- [lib/geo/ai-citability.ts:22-53](file://lib/geo/ai-citability.ts#L22-L53)