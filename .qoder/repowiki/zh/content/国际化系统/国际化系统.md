# 国际化系统

<cite>
**本文引用的文件**
- [messages/en.json](file://messages/en.json)
- [messages/zh.json](file://messages/zh.json)
- [messages/id.json](file://messages/id.json)
- [messages/th.json](file://messages/th.json)
- [messages/vi.json](file://messages/vi.json)
- [messages/ar.json](file://messages/ar.json)
- [middleware.ts](file://middleware.ts)
- [lib/i18n/config.ts](file://lib/i18n/config.ts)
- [app/[locale]/layout.tsx](file://app/[locale]/layout.tsx)
- [app/[locale]/page.tsx](file://app/[locale]/page.tsx)
- [app/[locale]/glossary/page.tsx](file://app/[locale]/glossary/page.tsx)
- [components/layout/navbar.tsx](file://components/layout/navbar.tsx)
- [components/layout/footer.tsx](file://components/layout/footer.tsx)
- [components/products/ai-citability-block.tsx](file://components/products/ai-citability-block.tsx)
- [data/led-glossary.ts](file://data/led-glossary.ts)
</cite>

## 更新摘要
**变更内容**
- 新增四种语言支持：印尼语(id)、泰语(th)、越南语(vi)、阿拉伯语(ar)
- 新增术语表系统（Glossary）章节，详细介绍AI优化的行业术语定义功能
- 更新导航栏组件，添加术语表链接
- 新增术语表页面的多语言支持说明
- 扩展术语表数据结构和AI优化内容展示组件
- 更新架构图和组件关系图

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [术语表系统](#术语表系统)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件系统化梳理 GoPro Trade 网站的国际化系统，基于 Next.js App Router 的路由与中间件能力，结合静态参数生成与动态消息加载，实现多语言内容渲染与语言切换。系统支持语言检测、URL 路由、RTL 语言适配、SEO 多语言链接、以及默认语言与回退策略。**新增的术语表系统**提供了AI优化的行业术语定义功能，支持多语言的术语解释，增强了网站的专业性和SEO表现。本文档面向开发者与产品/运营人员，既提供代码级实现解析，也给出新增语言与优化实践建议。

## 项目结构
国际化相关的关键目录与文件如下：
- 语言消息文件：messages/*.json（英文、中文、印尼语、泰语、越南语、阿拉伯语）
- 应用层路由与布局：app/[locale]/（按语言分段的页面与通用布局）
- 术语表系统：app/[locale]/glossary/（AI优化术语定义页面）
- 数据层：data/led-glossary.ts（术语定义数据库与搜索功能）
- 组件层：components/products/ai-citability-block.tsx（AI可引用内容展示）
- 中间件：middleware.ts（根路径语言检测与重定向）
- 全局根布局：app/layout.tsx（默认 html lang）
- 组件：components/layout/navbar.tsx、footer.tsx（语言切换 UI 与本地化文案）
- 构建配置：next.config.mjs（性能与安全优化）

```mermaid
graph TB
A["浏览器请求<br/>/ 或 /<locale>"] --> B["中间件<br/>middleware.ts"]
B --> |根路径检测| C["根据 Accept-Language 重定向<br/>/zh /en /id /th /vi /ar"]
A --> D["App Router 路由匹配<br/>app/[locale]/*"]
D --> E["语言布局<br/>app/[locale]/layout.tsx"]
E --> F["消息加载<br/>messages/<locale>.json"]
E --> G["页面组件<br/>app/[locale]/page.tsx"]
E --> H["术语表页面<br/>app/[locale]/glossary/page.tsx"]
G --> I["导航栏组件<br/>components/layout/navbar.tsx"]
G --> J["页脚组件<br/>components/layout/footer.tsx"]
H --> K["术语数据<br/>data/led-glossary.ts"]
H --> L["AI可引用内容块<br/>components/products/ai-citability-block.tsx"]
```

**图表来源**
- [middleware.ts:44-67](file://middleware.ts#L44-L67)
- [app/[locale]/layout.tsx:42-70](file://app/[locale]/layout.tsx#L42-L70)
- [app/[locale]/page.tsx:23-77](file://app/[locale]/page.tsx#L23-L77)
- [app/[locale]/glossary/page.tsx:1-167](file://app/[locale]/glossary/page.tsx#L1-L167)
- [components/layout/navbar.tsx:28-214](file://components/layout/navbar.tsx#L28-L214)
- [components/layout/footer.tsx:36-169](file://components/layout/footer.tsx#L36-L169)
- [data/led-glossary.ts:1-149](file://data/led-glossary.ts#L1-L149)
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)

**章节来源**
- [middleware.ts:1-68](file://middleware.ts#L1-L68)
- [app/[locale]/layout.tsx:1-71](file://app/[locale]/layout.tsx#L1-L71)
- [app/[locale]/page.tsx:1-334](file://app/[locale]/page.tsx#L1-L334)
- [app/[locale]/glossary/page.tsx:1-167](file://app/[locale]/glossary/page.tsx#L1-L167)
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)
- [messages/en.json:1-200](file://messages/en.json#L1-L200)
- [messages/zh.json:1-200](file://messages/zh.json#L1-L200)
- [messages/id.json:1-200](file://messages/id.json#L1-L200)
- [messages/th.json:1-200](file://messages/th.json#L1-L200)
- [messages/vi.json:1-200](file://messages/vi.json#L1-L200)
- [messages/ar.json:1-200](file://messages/ar.json#L1-L200)
- [data/led-glossary.ts:1-149](file://data/led-glossary.ts#L1-L149)
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)

## 核心组件
- 语言检测与重定向：middleware.ts 基于 Accept-Language 与预设映射表进行根路径重定向，并设置无缓存响应头。
- 语言布局与消息加载：app/[locale]/layout.tsx 动态加载对应语言消息，注入 html lang 与 dir（RTL 支持），生成 SEO hreflang。
- 首页与页面渲染：app/[locale]/page.tsx 动态读取消息键值，生成 SEO 元数据与结构化数据，按需加载产品数据。
- **术语表系统**：app/[locale]/glossary/page.tsx 提供AI优化的术语定义查询，支持搜索和分类筛选。
- **AI可引用内容块**：components/products/ai-citability-block.tsx 展示优化后的术语解释，支持展开/收起功能。
- **术语数据管理**：data/led-glossary.ts 提供术语定义、搜索、分类和AI优化内容生成。
- 导航与页脚：components/layout/navbar.tsx、footer.tsx 展示本地化文案，支持语言切换与本地化链接。
- 默认语言与全局根布局：app/layout.tsx 提供默认 html lang，next.config.mjs 提供性能与安全配置。

**章节来源**
- [middleware.ts:21-67](file://middleware.ts#L21-L67)
- [app/[locale]/layout.tsx:11-70](file://app/[locale]/layout.tsx#L11-L70)
- [app/[locale]/page.tsx:23-86](file://app/[locale]/page.tsx#L23-L86)
- [app/[locale]/glossary/page.tsx:20-167](file://app/[locale]/glossary/page.tsx#L20-L167)
- [components/products/ai-citability-block.tsx:21-90](file://components/products/ai-citability-block.tsx#L21-L90)
- [data/led-glossary.ts:95-141](file://data/led-glossary.ts#L95-L141)
- [components/layout/navbar.tsx:28-214](file://components/layout/navbar.tsx#L28-L214)
- [components/layout/footer.tsx:36-169](file://components/layout/footer.tsx#L36-L169)
- [app/layout.tsx:1-19](file://app/layout.tsx#L1-L19)
- [next.config.mjs:1-65](file://next.config.mjs#L1-L65)

## 架构总览
系统采用"中间件 + App Router 路由 + 动态消息加载"的组合：
- 中间件负责根路径的语言检测与重定向，避免 SSR/CSR 冲突。
- App Router 通过动态段 [locale] 匹配语言，生成静态参数，保证每个语言版本独立。
- 语言布局统一加载对应消息文件，按需渲染页面与组件。
- **术语表系统**通过专用路由提供AI优化的术语查询功能，支持多语言显示。
- 导航与页脚组件复用消息键值，实现一致的本地化体验。

```mermaid
sequenceDiagram
participant U as "用户"
participant MW as "中间件"
participant RT as "App Router"
participant LO as "语言布局"
participant GP as "首页页面"
participant GL as "术语表页面"
participant NB as "导航栏"
participant FT as "页脚"
U->>MW : 访问 "/"
MW->>MW : 解析 Accept-Language
MW->>U : 302 重定向至 "/<locale>"
U->>RT : 请求 "/<locale>"
RT->>LO : 渲染语言布局
LO->>LO : 动态加载 messages/<locale>.json
LO->>GP : 传递消息与 locale
LO->>GL : 传递术语表数据与 locale
GP->>NB : 渲染导航栏含语言切换
GP->>FT : 渲染页脚含本地化链接
GL->>NB : 渲染导航栏含术语表链接
GL->>FT : 渲染页脚含本地化链接
```

**图表来源**
- [middleware.ts:44-67](file://middleware.ts#L44-L67)
- [app/[locale]/layout.tsx:34-55](file://app/[locale]/layout.tsx#L34-L55)
- [app/[locale]/page.tsx:23-77](file://app/[locale]/page.tsx#L23-L77)
- [app/[locale]/glossary/page.tsx:20-167](file://app/[locale]/glossary/page.tsx#L20-L167)
- [components/layout/navbar.tsx:65-68](file://components/layout/navbar.tsx#L65-L68)
- [components/layout/footer.tsx:36-169](file://components/layout/footer.tsx#L36-L169)

## 详细组件分析

### 语言检测与重定向（middleware.ts）
- 接受浏览器请求头 Accept-Language，解析优先级，映射到已支持语言集合。
- 对根路径 "/" 进行 302 临时重定向，同时设置 no-store/no-cache/pragma/expire 响应头，避免代理缓存。
- 仅对根路径生效，其他路径透传。

```mermaid
flowchart TD
Start(["进入中间件"]) --> CheckPath{"是否为根路径 '/'?"}
CheckPath -- 否 --> Pass["透传请求"] --> End
CheckPath -- 是 --> Parse["解析 Accept-Language<br/>提取语言与优先级"]
Parse --> Map["按映射表匹配支持语言"]
Map --> Found{"是否匹配到支持语言?"}
Found -- 否 --> Default["使用默认语言"]
Found -- 是 --> Default
Default --> Build["构造新 URL '/<locale>'"]
Build --> SetHeaders["设置无缓存响应头"]
SetHeaders --> Redirect["302 临时重定向"]
Redirect --> End(["结束"])
```

**图表来源**
- [middleware.ts:21-67](file://middleware.ts#L21-L67)

**章节来源**
- [middleware.ts:1-68](file://middleware.ts#L1-L68)

### 语言布局与消息加载（app/[locale]/layout.tsx）
- generateStaticParams：为每个 locales 生成静态参数，确保语言版本可被预构建。
- generateMetadata：为每种语言生成 canonical 与 hreflang 列表，提升 SEO。
- getMessages：动态导入对应语言消息文件，异常时触发 404。
- 语言 HTML 标签与方向：根据 rtlLocales 注入 lang 与 dir（如 'rtl'）。
- 传递消息给 Navbar 与 Footer，实现导航与页脚的本地化。

```mermaid
sequenceDiagram
participant RT as "App Router"
participant LO as "语言布局"
participant MSG as "消息文件"
participant NB as "导航栏"
participant FT as "页脚"
RT->>LO : 传入 { locale }
LO->>LO : 校验 locale 是否在支持列表
LO->>MSG : 动态导入 messages/<locale>.json
MSG-->>LO : 返回消息对象
LO->>NB : 传递 navigation/common 消息
LO->>FT : 传递 navigation/footer/common 消息
LO-->>RT : 渲染 html/lang/dir 与页面内容
```

**图表来源**
- [app/[locale]/layout.tsx:11-70](file://app/[locale]/layout.tsx#L11-L70)

**章节来源**
- [app/[locale]/layout.tsx:1-71](file://app/[locale]/layout.tsx#L1-L71)

### 首页与页面渲染（app/[locale]/page.tsx）
- generateMetadata：基于消息文件动态生成标题、描述、OpenGraph 与 Twitter 卡片，并输出 hreflang。
- 消息映射：messagesMap 将 locale 映射到对应消息模块，作为 t(key) 函数使用。
- SEO 结构化数据：生成组织、网站、FAQ、本地商业等 Schema。
- 产品预览：按固定 slug 列表获取代表性产品，优先加载首图以优化 LCP。
- revalidate：ISR 配置为 1 小时，平衡新鲜度与性能。

```mermaid
flowchart TD
Enter(["进入首页"]) --> GetLocale["获取 locale 参数"]
GetLocale --> LoadMsg["加载 messagesMap[locale]"]
LoadMsg --> Meta["生成 SEO 元数据与 hreflang"]
Meta --> Schema["生成结构化数据"]
Schema --> FetchProducts["按固定 slug 列表获取产品"]
FetchProducts --> Render["渲染 Hero/TrustBar/Products 预览"]
Render --> Exit(["完成"])
```

**图表来源**
- [app/[locale]/page.tsx:23-150](file://app/[locale]/page.tsx#L23-L150)

**章节来源**
- [app/[locale]/page.tsx:1-334](file://app/[locale]/page.tsx#L1-L334)

### 导航与页脚（components/layout/navbar.tsx、footer.tsx）
- 导航栏：展示本地化菜单项与语言切换按钮，点击设置 Cookie 保存语言偏好；移动端与桌面端分别渲染。**新增术语表链接**。
- 页脚：展示本地化版权、快速链接、联系方式与认证标识，支持 RTL 方向。
- 本地化链接：根据当前路径与 locale 生成带语言前缀的链接，保持用户所在页面的语言上下文。

```mermaid
classDiagram
class Navbar {
+props : locale, messages
+handleLanguageChange(newLocale)
+getLocalizedHref(path)
+getPathWithoutLocale()
}
class Footer {
+props : locale, messages
+getLocalizedHref(path)
}
class I18nConfig {
+locales
+localeNames
+rtlLocales
}
Navbar --> I18nConfig : "使用"
Footer --> I18nConfig : "使用"
```

**图表来源**
- [components/layout/navbar.tsx:28-214](file://components/layout/navbar.tsx#L28-L214)
- [components/layout/footer.tsx:36-169](file://components/layout/footer.tsx#L36-L169)

**章节来源**
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)

### 语言文件组织与命名规范
- 文件命名：messages/<locale>.json，其中 <locale> 与 locales 配置一致。
- 键值结构：按功能域划分（如 navigation、hero、products、contact、footer、news、common），便于组件按需取用。
- 占位符与复数：消息文件中包含占位符与状态文案（如提交状态、数量区间），组件通过键路径读取。
- 示例键路径：navigation.home、products.categories.opticalSensor、common.loading 等。

**章节来源**
- [messages/en.json:1-200](file://messages/en.json#L1-L200)
- [messages/zh.json:1-200](file://messages/zh.json#L1-L200)
- [messages/id.json:1-200](file://messages/id.json#L1-L200)
- [messages/th.json:1-200](file://messages/th.json#L1-L200)
- [messages/vi.json:1-200](file://messages/vi.json#L1-L200)
- [messages/ar.json:1-200](file://messages/ar.json#L1-L200)

### RTL 语言支持（阿拉伯语等）
- 检测与注入：语言布局根据 rtlLocales 判断是否为 RTL 语言，并在 html 上设置 dir="rtl"。
- 组件适配：导航栏与页脚在 RTL 场景下应用相应样式类，确保布局正确。
- 注意：当前代码中页脚对特定语言（如 'ar'）做了显式判断，建议统一使用 rtlLocales 以减少分支。

**章节来源**
- [app/[locale]/layout.tsx:55-69](file://app/[locale]/layout.tsx#L55-L69)
- [components/layout/footer.tsx:37-49](file://components/layout/footer.tsx#L37-L49)

### 语言回退与默认语言
- 默认语言：中间件与消息加载均以 defaultLocale 作为回退。
- 回退策略：当浏览器语言无法匹配或消息文件缺失时，系统回退到默认语言（通常为英语）。
- 建议：在新增语言时，先提供基础键值，再逐步完善翻译，确保回退路径稳定。

**章节来源**
- [middleware.ts:3, 21-42:3-42](file://middleware.ts#L3-L42)
- [app/[locale]/layout.tsx:34-40](file://app/[locale]/layout.tsx#L34-L40)

### URL 模式与 SEO
- URL 模式：/ 或 /<locale>，页面内链接统一使用带语言前缀的路径。
- hreflang：语言布局为每种语言生成 canonical 与 hreflang 列表，提升多语言 SEO。
- OpenGraph/Twitter：首页动态生成对应语言的元数据与卡片，增强分享效果。

**章节来源**
- [app/[locale]/layout.tsx:16-31](file://app/[locale]/layout.tsx#L16-L31)
- [app/[locale]/page.tsx:23-77](file://app/[locale]/page.tsx#L23-L77)

## 术语表系统

### 系统概述
术语表系统是一个AI优化的行业术语定义功能，专门为LED行业提供专业的技术术语解释。该系统支持多语言显示，通过AI优化的内容结构，提升在AI搜索引擎中的可见性。

### 核心功能
- **AI优化术语定义**：每个术语都提供134-167词的AI引用优化版本
- **多语言支持**：与网站国际化系统集成，支持所有已支持语言
- **智能搜索**：支持按术语、定义和关键词进行搜索
- **分类筛选**：按技术类别对术语进行组织和筛选
- **展开式展示**：支持展开/收起功能，提供详细信息

### 数据结构
术语表使用统一的数据接口，包含以下字段：
- `term`：术语名称
- `category`：所属技术类别
- `definition`：标准定义
- `aiOptimized`：AI优化版本（134-167词）
- `keywords`：关键词数组
- `relatedTerms`：相关术语
- `applications`：应用场景

### 组件架构
- **术语表页面**：app/[locale]/glossary/page.tsx - 主要入口，提供搜索和筛选功能
- **AI可引用内容块**：components/products/ai-citability-block.tsx - 展示优化后的内容
- **术语数据管理**：data/led-glossary.ts - 提供术语定义、搜索和分类功能

```mermaid
graph TB
A["术语表页面<br/>app/[locale]/glossary/page.tsx"] --> B["术语数据层<br/>data/led-glossary.ts"]
B --> C["术语定义库<br/>LED_GLOSSARY"]
B --> D["搜索功能<br/>searchGlossary()"]
B --> E["分类功能<br/>getTermsByCategory()"]
B --> F["AI优化<br/>getAIOptimizedDefinition()"]
A --> G["AI可引用内容块<br/>components/products/ai-citability-block.tsx"]
G --> H["展开/收起功能"]
G --> I["关键词标签"]
G --> J["关键事实列表"]
```

**图表来源**
- [app/[locale]/glossary/page.tsx:20-167](file://app/[locale]/glossary/page.tsx#L20-L167)
- [data/led-glossary.ts:17-141](file://data/led-glossary.ts#L17-L141)
- [components/products/ai-citability-block.tsx:21-90](file://components/products/ai-citability-block.tsx#L21-L90)

### 多语言实现
术语表系统完全集成到现有的国际化框架中：
- **路由集成**：通过App Router的动态段 [locale] 实现多语言路由
- **消息加载**：自动加载对应语言的消息文件
- **内容本地化**：术语表界面元素使用本地化消息
- **SEO优化**：为每种语言生成独立的SEO元数据

### AI优化策略
术语表内容经过专门的AI优化，采用以下策略：
- **结构化格式**：提供清晰的问题-答案格式
- **关键数据点**：包含类别、相关术语、应用场景等关键信息
- **关键词密度**：合理分布关键词，提升搜索引擎优化效果
- **专业术语**：使用行业标准术语，确保准确性

**章节来源**
- [app/[locale]/glossary/page.tsx:1-167](file://app/[locale]/glossary/page.tsx#L1-L167)
- [data/led-glossary.ts:1-149](file://data/led-glossary.ts#L1-L149)
- [components/products/ai-citability-block.tsx:1-178](file://components/products/ai-citability-block.tsx#L1-L178)

## 依赖关系分析
- 中间件依赖 locales 与 defaultLocale，控制根路径重定向。
- 语言布局依赖消息文件与 i18n 配置，生成元数据与渲染组件。
- 页面组件依赖消息映射与结构化数据工具，负责内容渲染与 SEO。
- **术语表系统**依赖术语数据层与AI可引用组件，提供专业术语解释。
- 导航与页脚依赖 i18n 配置与消息，负责语言切换与本地化链接。

```mermaid
graph LR
MW["middleware.ts"] --> CFG["i18n 配置"]
LO["app/[locale]/layout.tsx"] --> MSG["messages/*.json"]
PG["app/[locale]/page.tsx"] --> MSG
GL["app/[locale]/glossary/page.tsx"] --> DATA["data/led-glossary.ts"]
GL --> COMP["components/products/ai-citability-block.tsx"]
NB["components/layout/navbar.tsx"] --> CFG
FT["components/layout/footer.tsx"] --> CFG
PG --> NB
PG --> FT
GL --> NB
GL --> FT
```

**图表来源**
- [middleware.ts:3](file://middleware.ts#L3)
- [app/[locale]/layout.tsx:3, 34-40](file://app/[locale]/layout.tsx#L3-L40)
- [app/[locale]/page.tsx:12-18](file://app/[locale]/page.tsx#L12-L18)
- [app/[locale]/glossary/page.tsx:9, 10](file://app/[locale]/glossary/page.tsx#L9-L10)
- [data/led-glossary.ts:17](file://data/led-glossary.ts#L17)
- [components/products/ai-citability-block.tsx:21](file://components/products/ai-citability-block.tsx#L21)
- [components/layout/navbar.tsx:6](file://components/layout/navbar.tsx#L6)
- [components/layout/footer.tsx:2](file://components/layout/footer.tsx#L2)

**章节来源**
- [middleware.ts:1-68](file://middleware.ts#L1-L68)
- [app/[locale]/layout.tsx:1-71](file://app/[locale]/layout.tsx#L1-L71)
- [app/[locale]/page.tsx:1-334](file://app/[locale]/page.tsx#L1-L334)
- [app/[locale]/glossary/page.tsx:1-167](file://app/[locale]/glossary/page.tsx#L1-L167)
- [components/layout/navbar.tsx:1-215](file://components/layout/navbar.tsx#L1-L215)
- [components/layout/footer.tsx:1-170](file://components/layout/footer.tsx#L1-L170)

## 性能考量
- 图片优化：next.config.mjs 启用现代图片格式与懒加载缓存，提升 LCP 指标。
- 压缩与安全：启用 gzip 压缩，隐藏 X-Powered-By，设置安全响应头。
- ISR 与缓存：首页 revalidate=3600，**术语表 revalidate=86400（24小时）**，减少频繁拉取数据；中间件设置无缓存，避免语言重定向被代理缓存。
- 消息加载：按语言动态导入消息文件，避免一次性加载全部语言包。
- **术语表优化**：使用ISR缓存，减少AI优化内容的重复计算。

**章节来源**
- [next.config.mjs:4-61](file://next.config.mjs#L4-L61)
- [app/[locale]/page.tsx:149-150](file://app/[locale]/page.tsx#L149-L150)
- [app/[locale]/glossary/page.tsx:17-18](file://app/[locale]/glossary/page.tsx#L17-L18)
- [middleware.ts:56-62](file://middleware.ts#L56-L62)

## 故障排查指南
- 404 语言页面：当 locale 不在支持列表或消息文件不存在时，语言布局触发 notFound。
- 语言切换无效：检查导航栏 handleLanguageChange 是否设置 Cookie，以及链接是否包含正确 locale 前缀。
- SEO 多语言链接缺失：确认语言布局 generateMetadata 是否正确生成 hreflang 与 canonical。
- RTL 布局异常：确认 rtlLocales 配置与 html dir 注入逻辑，避免硬编码特定语言判断。
- **术语表显示问题**：检查术语数据是否正确加载，AI优化内容是否可用，搜索功能是否正常工作。
- **术语表缓存问题**：确认 revalidate 配置是否正确，检查ISR缓存是否过期。

**章节来源**
- [app/[locale]/layout.tsx:50-52](file://app/[locale]/layout.tsx#L50-L52)
- [components/layout/navbar.tsx:35-40](file://components/layout/navbar.tsx#L35-L40)
- [app/[locale]/layout.tsx:16-31](file://app/[locale]/layout.tsx#L16-L31)
- [components/layout/footer.tsx:37-49](file://components/layout/footer.tsx#L37-L49)
- [app/[locale]/glossary/page.tsx:95-118](file://app/[locale]/glossary/page.tsx#L95-L118)

## 结论
该国际化系统通过中间件、App Router 动态段与动态消息加载实现了稳定的多语言支持，具备良好的 SEO 与性能表现。**新增的术语表系统**进一步增强了网站的专业性，通过AI优化的术语定义提升了在AI搜索引擎中的可见性。系统支持完整的RTL语言适配，提供一致的用户体验。建议在新增语言时遵循现有文件与键值规范，确保消息文件完整性与 RTL 适配一致性，并持续优化 ISR 与缓存策略以提升用户体验。

## 附录

### 新增语言支持操作指南
- 创建消息文件：在 messages 目录新增 <new_locale>.json，复制现有语言结构，逐项翻译。
- 更新支持列表：在 i18n 配置中加入新语言代码与名称映射。
- 生成静态参数：确保语言布局 generateStaticParams 能生成新语言参数。
- 验证 SEO：在语言布局与页面组件中确认 hreflang 与元数据生成正常。
- RTL 适配：如为目标语言启用 RTL，确保 rtlLocales 包含该语言并在布局中注入 dir="rtl"。
- 测试：访问根路径与各语言页面，检查消息加载、语言切换与链接生成。

### 术语表扩展指南
- 添加新术语：在 data/led-glossary.ts 的 LED_GLOSSARY 数组中添加新的术语条目。
- AI优化内容：为每个新术语提供134-167词的AI优化版本。
- 关键词设置：为术语设置相关的关键词，提升搜索效果。
- 分类管理：将术语归类到合适的类别中。
- 多语言翻译：为术语表界面元素提供多语言翻译支持。

**章节来源**
- [messages/en.json:1-200](file://messages/en.json#L1-L200)
- [app/[locale]/layout.tsx:11-13](file://app/[locale]/layout.tsx#L11-L13)
- [app/[locale]/layout.tsx:16-31](file://app/[locale]/layout.tsx#L16-L31)
- [app/[locale]/layout.tsx:55-59](file://app/[locale]/layout.tsx#L55-L59)
- [data/led-glossary.ts:17-90](file://data/led-glossary.ts#L17-L90)
- [app/[locale]/glossary/page.tsx:106-118](file://app/[locale]/glossary/page.tsx#L106-L118)