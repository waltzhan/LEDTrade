...
## 更新摘要
**所做更改**
- 更新了新闻自动化API的安全验证机制部分，反映了条件验证逻辑和改进的错误处理
- 新增了安全验证机制的详细说明，包括CRON_SECRET的条件验证和警告机制
- 更新了故障排查指南中关于安全验证的相关内容
...
INDEX["scripts/news-auto/index.ts"]
...
- [scripts/news-auto/crawler.ts:1-200](file://scripts/news-auto/crawler.ts#L1-L200)
...
API->>API : "条件验证 CRON_SECRET"
API->>Index : "runNewsAutomation()"
Index->>Scheduler : "shouldPublish()"
Scheduler-->>Index : "{should, remaining}"
Index->>Crawler : "crawlNews()"
Crawler-->>Index : "RawArticle[]"
Index->>AI : "processArticles()"
AI-->>Index : "ProcessedArticle[]"
Index->>Publisher : "publishArticles()"
Publisher->>Sanity : "create 文章"
Publisher-->>Index : "发布结果"
Index-->>API : "执行结果"
API-->>Cron : "JSON 响应"
```

**图表来源**
- [app/api/cron/news/route.ts:5-57](file://app/api/cron/news/route.ts#L5-L57)
- [scripts/news-auto/index.ts:9-78](file://scripts/news-auto/index.ts#L9-L78)
- [scripts/news-auto/crawler.ts:163-200](file://scripts/news-auto/crawler.ts#L163-L200)
...
- 功能概述
  - 接收 Vercel Cron 触发，执行条件验证（CRON_SECRET），执行新闻自动化流程。
  - 校验 DASHSCOPE_API_KEY，确保 AI 改写与翻译可用。
  - 返回 JSON 结果，包含成功标志与时间戳；异常时返回 500 与错误信息。
- 安全与鉴权
  - Authorization 头需匹配 Bearer CRON_SECRET。
  - **新增** 条件验证机制：只有当环境变量中配置了 CRON_SECRET 时才进行严格验证；如果未配置，记录警告但不阻止执行，便于开发环境使用。
  - 未配置 API Key 时直接拒绝。
- 并发与稳定性
  - 捕获异常并记录日志，避免中断。
- HTTP 方法
...
```mermaid
flowchart TD
Start(["请求进入"]) --> CheckEnv["检查 CRON_SECRET 环境变量"]
CheckEnv --> |存在| Auth["校验 Authorization 头"]
CheckEnv --> |不存在| Warn["记录警告但继续执行"]
...
**图表来源**
- [app/api/cron/news/route.ts:6-57](file://app/api/cron/news/route.ts#L6-L57)
...
- 主流程（index.ts）
  - **新增** 完整的自动化流程控制，包括发布检查、抓取、AI处理、发布摘要输出。
  - 提供详细的日志输出，包括任务开始时间、北京时间、抓取数量、AI处理数量、发布成功数量等。
- 抓取器（crawler）
...
+runNewsAutomation() Promise~void~
...
NewsAutomation --> Scheduler : "提供发布检查"
NewsAutomation --> Crawler : "提供原始文章"
...
F --> G["Sanity"]
H["/api/sitemap"] --> G
I["/api/inquiry"] --> J["wechat.ts"]
I --> K["sanity/inquiry.ts"]
L["Vercel Cron"] --> A
M["Rewrite sitemap.xml -> /api/sitemap"] --> H
```
...
## 故障排查指南
- 新闻自动化失败
  - **更新** 检查 CRON_SECRET 是否正确配置与传递。注意：如果未配置 CRON_SECRET，系统会记录警告但不会阻止执行，这在开发环境中是预期行为。
  - 确认 DASHSCOPE_API_KEY 是否存在。
  - 查看日志中的错误堆栈与具体失败步骤。
  - **新增** 检查 NEWS_BYPASS_TIME_CHECK 环境变量，确认是否启用了本地测试模式。
- 站点地图为空
...
**章节来源**
- [app/api/cron/news/route.ts:18-21](file://app/api/cron/news/route.ts#L18-L21)
...
## 结论
本 API 系统围绕"定时抓取—AI 处理—发布入库—多语言站点地图—询盘通知"形成闭环，具备良好的扩展性与可维护性。通过独立配置文件管理新闻源、**增强的安全验证机制（条件验证逻辑）**、严格的鉴权与限流策略、并行处理与缓存优化，满足生产环境对稳定性与性能的要求。
...
- 新闻自动化（定时触发）
  ...
  - **新增** 开发环境可设置 NEWS_BYPASS_TIME_CHECK=true 绕过时间检查。
- 站点地图
...
- 请求鉴权
  ...
  - **更新** 采用条件验证机制：仅在配置了 CRON_SECRET 时进行严格验证，未配置时记录警告但允许执行，便于开发环境使用。
- 数据校验
...
- [app/api/cron/news/route.ts:10-21](file://app/api/cron/news/route.ts#L10-L21)
