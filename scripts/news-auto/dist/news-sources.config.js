/**
 * 新闻源独立配置文件
 * ============================================================
 * 维护指南：
 *   - 新增新闻源：在 NEWS_SOURCES 数组中添加新对象
 *   - 停用新闻源：设置 enabled: false（无需删除）
 *   - 调整优先级：修改 priority 值（数字越小优先级越高）
 *   - 仅RSS：设置 type: 'rss'，填写 rss 字段
 *   - 仅网页：设置 type: 'web'，填写 selector 字段
 * ============================================================
 */
/**
 * 新闻源配置列表
 * 维护时只需增删改此数组，无需改动抓取逻辑代码
 */
export var NEWS_SOURCES = [
    // ── 中文新闻源 ─────────────────────────────────────────────
    {
        name: 'LEDinside',
        url: 'https://www.ledinside.com',
        type: 'rss',
        rss: 'https://www.ledinside.com/rss.xml',
        category: 'industry',
        language: 'zh',
        priority: 1,
        enabled: true,
        notes: '主要中文 LED 行业媒体，RSS 稳定可用',
    },
    {
        name: '半导体照明网',
        url: 'http://www.china-led.net',
        type: 'web',
        selector: '.news-item',
        category: 'technical',
        language: 'zh',
        priority: 2,
        enabled: true,
        notes: '技术专业媒体，已启用',
    },
    {
        name: 'OFweek 半导体照明',
        url: 'https://led.ofweek.com',
        type: 'rss',
        rss: 'https://led.ofweek.com/rss.xml',
        category: 'technical',
        language: 'zh',
        priority: 3,
        enabled: true,
        notes: '技术专业媒体，RSS 可用',
    },
    {
        name: '百度新闻 - LED/半导体',
        url: 'https://news.baidu.com',
        type: 'rss',
        rss: 'https://news.baidu.com/ns?word=LED+%E5%8D%8A%E5%AF%BC%E4%BD%93&tn=news&rn=20&ie=utf-8&out=utf-8',
        category: 'industry',
        language: 'zh',
        priority: 4,
        enabled: true,
        notes: '百度新闻聚合搜索，关键词：LED 半导体',
    },
    // ── 英文新闻源 ─────────────────────────────────────────────
    {
        name: 'Google News - LED Technology',
        url: 'https://news.google.com',
        type: 'rss',
        rss: 'https://news.google.com/rss/search?q=LED+technology+OR+semiconductor+OR+optoelectronics&hl=en-US&gl=US&ceid=US:en',
        category: 'industry',
        language: 'en',
        priority: 5,
        enabled: true,
        notes: '谷歌新闻聚合搜索，关键词：LED technology, semiconductor, optoelectronics',
    },
    {
        name: 'LEDs Magazine',
        url: 'https://www.ledsmagazine.com',
        type: 'rss+web',
        rss: 'https://www.ledsmagazine.com/rss.xml',
        selector: '.article-teaser',
        category: 'industry',
        language: 'en',
        priority: 6,
        enabled: true,
        notes: '英文行业媒体，启用 RSS+Web 双模式',
        headers: {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        },
    },
    {
        name: 'Google News - Mini/Micro LED',
        url: 'https://news.google.com',
        type: 'rss',
        rss: 'https://news.google.com/rss/search?q=Mini+LED+OR+Micro+LED+OR+display+technology&hl=en-US&gl=US&ceid=US:en',
        category: 'technical',
        language: 'en',
        priority: 7,
        enabled: true,
        notes: '谷歌新闻聚合搜索，关键词：Mini LED, Micro LED, display technology',
    },
    {
        name: 'Compound Semiconductor',
        url: 'https://www.compoundsemiconductor.net',
        type: 'rss',
        rss: 'https://www.compoundsemiconductor.net/rss.xml',
        category: 'technical',
        language: 'en',
        priority: 8,
        enabled: true,
        notes: '技术专业媒体，已启用',
    },
    {
        name: 'Laser Focus World',
        url: 'https://www.laserfocusworld.com',
        type: 'rss',
        rss: 'https://www.laserfocusworld.com/rss.xml',
        category: 'technical',
        language: 'en',
        priority: 7,
        enabled: false,
        notes: '光电技术专业媒体，待测试',
    },
];
/**
 * 获取所有已启用的新闻源（按优先级排序）
 */
export function getEnabledSources() {
    return NEWS_SOURCES
        .filter(function (s) { return s.enabled; })
        .sort(function (a, b) { return a.priority - b.priority; });
}
/**
 * 获取指定分类的已启用新闻源
 */
export function getSourcesByCategory(category) {
    return getEnabledSources().filter(function (s) { return s.category === category; });
}
/**
 * 获取指定语言的已启用新闻源
 */
export function getSourcesByLanguage(language) {
    return getEnabledSources().filter(function (s) { return s.language === language; });
}
